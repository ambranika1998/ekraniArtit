...
class MenuTypeChoices(models.TextChoices):
    ...
    PROGRAMME = 'programme', gettext_lazy('Programme')
...
class Sponsor(models.Model):
    ...
    youtube = models.URLField(gettext_lazy("Youtube"), max_length=200, db_index=True, null=True, blank=True)
    linkedin = models.URLField(gettext_lazy("Linkedin"), max_length=200, db_index=True, null=True, blank=True)
    tiktok = models.URLField(gettext_lazy("Tiktok"), max_length=200, db_index=True, null=True, blank=True)
    order = models.PositiveIntegerField(verbose_name=gettext_lazy('order'), default=1)
...
class DayDate(models.Model):
    class Meta:
        verbose_name = gettext_lazy("day date")
        verbose_name_plural = gettext_lazy("day dates")
        db_table = "day_date"

    day = models.CharField(verbose_name=gettext_lazy("day"), max_length=128, blank=False, null=False)
    day_al = models.CharField(verbose_name=gettext_lazy("day albanian"), max_length=128, blank=False, null=False)
    date = models.CharField(verbose_name=gettext_lazy("date"), max_length=128, blank=False, null=False)
    date_al = models.CharField(verbose_name=gettext_lazy("date albanian"), max_length=128, blank=False, null=False)
    order = models.PositiveIntegerField(verbose_name=gettext_lazy('order'), default=1)
...
class DayProgrammes(models.Model):
    class Meta:
        verbose_name = gettext_lazy("day programme")
        verbose_name_plural = gettext_lazy("day programmes")
        db_table = "day_programme"

    day_date = models.ForeignKey(DayDate, related_name='day_programmes', on_delete=models.CASCADE, verbose_name=gettext_lazy('day date'))
